	...
	<view>
		...
		<view class="uni-padding-wrap uni-common-mt">
			...
			<view class="uni-btn-v">
				...
				<view v-if="notificationData">
					<view class="list-name">
						最新的值:{{ notificationData || '还没有最新值' }}
					</view>
				</view>
				<button v-if="notificationData" type="primary" :disabled="chartMaskShow" @click="showChart">
					显示波形图
				</button>
		...
		<view v-if="chartMaskShow" class="uni-mask" @touchmove.stop.prevent="chartMoveHandle" @click="chartMaskClose">
...
<script>
	...
	export default {
		...
		methods: {
			...
			showChart() {
				this.chartMaskShow = true;
			},
			chartMaskClose() {
				this.chartMaskShow = false;
			},
			/**
			 * 选择设备
			 */
			...
			onBLECharacteristicValueChange() {
				...
				uni.onBLECharacteristicValueChange(characteristic => {
					let valuehex = ab2hex(characteristic['value']);
					let value = parseInt(valuehex.substring(4, 6) + valuehex.substring(2, 4), 16);
					//这里我后台返回的是数组，所以用等于，如果您后台返回的是单条数据，需要push进去
					if(LineA.categories.length >= 20)
					{
						LineA.categories.shift();
						LineA.categories.push('');
					}
					else
						LineA.categories.push('');
					if(y.length >= 20)
